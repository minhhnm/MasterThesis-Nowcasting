...
class RadarLightningModel(pl.LightningModule):
    ...
    def shared_step(
        ...
        past = data[:, : -self.hparams.forecast_steps].float()
        future = data[:, -self.hparams.forecast_steps :].float()
    
        if mask is not None:
            mask = mask[:, -self.hparams.forecast_steps :].float()
    
        preds = self(
            past,
            forecast_steps=self.hparams.forecast_steps,
            ensemble_size=ensemble_size,
        ).clamp(min=-1, max=1)
    
        if self.hparams.masked_loss:
            if mask is None:
                raise ValueError("masked_loss=True but batch does not contain 'mask'")
        ...
        if split == "train" and (
            self.global_step in self.log_images_iterations
            or self.global_step % self.log_images_iterations[-1] == 0
        ):
