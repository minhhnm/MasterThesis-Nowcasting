...
class RadarDatasetNew(Dataset):
    def __init__(self,
        ...
        self._data_radar_pid = os.getpid()
        print("Init done", flush=True)

    def _ensure_data_open(self) -> None:
        """
        Make xarray/Zarr access safe with PyTorch DataLoader workers.

        The dataset is constructed in the parent process, but DataLoader workers
        are forked child processes. Inheriting already-open xarray/Zarr objects
        across fork can hang when the workers load the first batch. Therefore,
        each process reopens the Zarr store the first time it reads data.
        """
        pid = os.getpid()
        if getattr(self, "_data_radar_pid", None) == pid and getattr(self, "data_radar", None):
            return

        # Close inherited handles in the child process before reopening.
        for ds in getattr(self, "data_radar", {}).values():
            try:
                ds.close()
            except Exception:
                pass
        ...
        self.data_radar = {}
        for region in self.regions:
            self.data_radar[region] = self.open_dataset(
                self.dataset_sources[region]["radar"]["data_dir"]
            )
        self._data_radar_pid = pid
        print(f"RadarDatasetNew reopened radar data in pid={pid}", flush=True)
