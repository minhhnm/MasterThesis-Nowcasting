...
from radar_dataset_sampled_multiproc_safe import RadarDatasetNew
...
def main():
    ...
    print("About to read train_ds[0]...", flush=True)
    sample = train_ds[0]
    print("Finished reading train_ds[0]", flush=True)
    print("train[0] radar_past:", tuple(sample["radar_past"].shape), flush=True)
    print("train[0] radar_future:", tuple(sample["radar_future"].shape), flush=True)
    print("train[0] metadata:", sample["t"], sample["x"], sample["y"], flush=True)
    ...
    print("Testing first train_loader batch...", flush=True)
    t0 = time.time()
    xb, yb = next(iter(train_loader))
    print("First batch loaded in", time.time() - t0, "seconds", flush=True)
